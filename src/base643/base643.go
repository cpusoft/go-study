...
func main() {
	...
	fmt.Println(len(b1), err)

	b2, err := base64.RawStdEncoding.DecodeString(s)
	fmt.Println(len(b2), err)

	b3, err := base64.URLEncoding.DecodeString(s)
	fmt.Println(len(b3), err)

	err := base64util.baseWriteBase64ToFile(f, s)
}
