...
func (ts *TcpTlsServer) OnConnect(tcpTlsConn *TcpTlsConn) {
	...
	belogs.Debug("OnConnect(): tcpserver tcpConn: ", tcpTlsConn.RemoteAddr().String(), ", connKey:", connKey, "  new len(tcpTlsConns): ", len(ts.tcpTlsConns))
	ts.tcpTlsServerProcessFunc.OnConnectProcess(tcpTlsConn)
	belogs.Info("OnConnect(): tcpserver add tcpConn: ", tcpTlsConn.RemoteAddr().String(), "   len(tcpTlsConns): ", len(ts.tcpTlsConns), "   time(s):", time.Now().Sub(start).Seconds())
...
func (ts *TcpTlsServer) ReceiveAndSend(tcpTlsConn *TcpTlsConn) {
	...
	for {
		...
		start := time.Now()
		belogs.Debug("ReceiveAndSend(): tcpserver read: Read n: ", tcpTlsConn.RemoteAddr().String(), n)
		if err != nil {
			if err == io.EOF {
				// is not error, just client close
				belogs.Info("ReceiveAndSend(): tcpserver Read io.EOF, client close: ", tcpTlsConn.RemoteAddr().String(), err)
				return
			}
			belogs.Error("ReceiveAndSend(): tcpserver Read fail, err ", tcpTlsConn.RemoteAddr().String(), err)
			return
		}
		if n == 0 {
			continue
		...
		// copy to leftData
		belogs.Debug("ReceiveAndSend(): tcpserver  will ReceiveAndSendProcess, server tcpTlsConn: ", tcpTlsConn.RemoteAddr().String(), "  n:", n,
		...
		belogs.Debug("ReceiveAndSend(): tcpserver  after ReceiveAndSendProcess,server tcpTlsConn: ", tcpTlsConn.RemoteAddr().String(), " receive n: ", n,
			"  len(leftData):", len(leftData), "  time(s):", time.Now().Sub(start))
		if err != nil {
			belogs.Error("OnReceiveAndSend(): tcpserver ReceiveAndSendProcess fail ,will remove this tcpTlsConn : ", tcpTlsConn.RemoteAddr().String(), err)
			return
		}
		if nextConnectPolicy == util.NEXT_CONNECT_POLICE_CLOSE_GRACEFUL ||
			nextConnectPolicy == util.NEXT_CONNECT_POLICE_CLOSE_FORCIBLE {
			belogs.Info("OnReceiveAndSend(): tcpserver  nextConnectPolicy return : ", tcpTlsConn.RemoteAddr().String(), nextConnectPolicy)
			return
		}
	}
}
...
func (ts *TcpTlsServer) OnClose(tcpTlsConn *TcpTlsConn) {
	...
	belogs.Debug("OnClose(): tcpserver server,tcpTlsConn: ", tcpTlsConn.RemoteAddr().String(), "   call process func: OnClose ")
	ts.tcpTlsServerProcessFunc.OnCloseProcess(tcpTlsConn)

	// remove tcpConn from tcpConns
	ts.tcpTlsConnsMutex.Lock()
	defer ts.tcpTlsConnsMutex.Unlock()
	belogs.Debug("OnClose(): tcpserver server,tcpTlsConn: ", tcpTlsConn.RemoteAddr().String(), "   old len(tcpTlsConns): ", len(ts.tcpTlsConns))
	...
	belogs.Info("OnClose(): tcpserver server,new len(tcpTlsConns): ", len(ts.tcpTlsConns), "  time(s):", time.Now().Sub(start).Seconds())
}
...
func (ts *TcpTlsServer) ActiveSend(sendData []byte, connKey string) (err error) {
	...
	belogs.Debug("ActiveSend(): tcpserver ,len(sendData):", len(sendData), "   len(tcpTlsConns): ", len(ts.tcpTlsConns), "  connKey:", connKey)
	if len(connKey) == 0 {
		belogs.Debug("ActiveSend(): tcpserver , to all, len(sendData):", len(sendData), "   len(tcpConns): ", len(ts.tcpTlsConns))
		for i := range ts.tcpTlsConns {
			belogs.Debug("ActiveSend(): tcpserver   to all, client: ", i, "    ts.tcpConns[i]:", ts.tcpTlsConns[i], "   call process func: ActiveSend ")
			err = ts.tcpTlsServerProcessFunc.ActiveSendProcess(ts.tcpTlsConns[i], sendData)
			if err != nil {
				// just logs, not return or break
				belogs.Error("ActiveSend(): tcpserver  ActiveSendProcess fail, to all, client: ", i, "    ts.tcpTlsConns[i]:", ts.tcpTlsConns[i], err)
			}
		}
		belogs.Info("ActiveSend(): tcpserver  send to all clients ok,  len(sendData):", len(sendData), "   len(tcpTlsConns): ", len(ts.tcpTlsConns),
		...
		if tcpTlsConn, ok := ts.tcpTlsConns[connKey]; ok {
			...
			if err != nil {
				// just logs, not return or break
				belogs.Error("ActiveSend(): tcpserver  fail, to connKey: ", connKey, "   tcpTlsConn:", tcpTlsConn.RemoteAddr().String(), err)
			}
		}
